...
- **Costs:** cost model 2 excludes unsuitable trails per profile before snapping/search, preserves OSM rideability tags, penalizes missing grades, and uses bilinear terrain heights with 80 m windows along complete ways. Directional slope samples, surfaces, deterministic road-stress proxies, uncertainty and 1 km reachable low-stress network utility. Attraction discounts are capped at 65%, retaining positive costs. Personalization is disabled in the baseline.
...
- **Offline:** checksummed immutable packs, gzip-JSON graph chunks with `.bin` names, PMTiles basemap, local source attribution, staged/resumable file downloads, OPFS with IndexedDB fallback, and a scoped production service worker. Routing and export work offline; the sole bundled custom style uses online MapTiler tiles, fonts, and sprites. The legacy PMTiles basemap remains in packs for compatibility.
- **Publication:** build-time S3 credentials only. `VITE_REGION_MANIFEST` selects the public pack. The map uses `VITE_MAPTILER_API_KEY` exclusively from the workspace `.env`; no ambient MapTiler credential is used.
