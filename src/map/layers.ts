...
export function addAppLayers(m: maplibregl.Map) {
  ...
  // The area a place search is limited to: an outline over the route, barely filled, so
  // the stretch it covers stays readable.
  m.addLayer({
    id: "search-area-fill",
    type: "fill",
    source: "search-area",
    paint: { "fill-color": CENTER_COLOR, "fill-opacity": 0.06 },
  });
  m.addLayer({
    id: "search-area-line",
    type: "line",
  ...
  // Notes and places: small at a whole-ride zoom, where a 500 km route carries hundreds.
  m.addLayer({
    id: "notes-dot",
    type: "circle",
    source: "notes",
